...
public class TrainerManager extends ThreadScheduler {
     ...
     */
    public TrainerManager(PreferencesManager prefManager, TimingsManager pManager, LinkedList<ExperimentData> expList,
                          HashMap<AlgorithmType, LinkedList<AlgorithmConfiguration>> confList, Metric metric,
                          Reputation reputation, DataCategory[] dataTypes, AlgorithmType[] algTypes) {
        super();
    ...
    private HashMap<String, String> readIndCombinations(String filename) {
        return readIndCombinations(new File(prefManager.getPreference(DetectionManager.SETUP_FILE_FOLDER)
                + filename));
    }

    private HashMap<String, String> readIndCombinations(File indCoupleFile) {
        HashMap<String, String> comb = new HashMap<>();
        ...
        try {
            if (indCoupleFile.exists()) {
                ...
                while (reader.ready()) {
                    ...
                    if (read != null) {
                        read = read.trim();
                        if (read.length() > 0 && !read.trim().startsWith("*") && read.contains(";")) {
                            comb.put(read.split(";")[0].trim(), read.split(";")[1].trim());
                        }
                    }
                }
                reader.close();
    ...
    private LinkedList<DataSeries> generateDataSeries(DataCategory[] dataTypes) {
        ...
        if (complexDataPreference.equals("ALL")) {
            ...
            return DataSeries.selectedCombinations(expList.getFirst().getIndicators(), dataTypes,
                    readPossibleIndCombinations());
        }
    }
    ...
    public void train() {
        ...
        try {
            ...
            pManager.addTiming(TimingsManager.AVG_TRAIN_TIME,
                    ((System.currentTimeMillis() - start) / threadNumber() * 1.0));
            AppLogger.logInfo(getClass(), "Training executed in " + (System.currentTimeMillis() - start)
                    + "ms");
    ...
    private LinkedList<? extends Thread> filterTrainers(LinkedList<? extends Thread> trainerList) {
        LinkedList<AlgorithmTrainer> invList = new LinkedList<>();
        if (iManager != null) {
    ...
    protected void initRun() {
        ...
        LinkedList<AlgorithmTrainer> trainerList = new LinkedList<>();
        for (AlgorithmType algType : algTypes) {
            if (confList.get(algType) != null) {
                switch (algType) {
                    case RCC:
                        trainerList.add(new FixedConfigurationTrainer(algType, null, metric, reputation,
                                tTiming, expList, confList.get(algType).getFirst()));
                        break;
                    case PEA:
                        PearsonCombinationManager pcManager;
                        File pearsonFile = new File(prefManager.getPreference(DetectionManager
                                .SETUP_FILE_FOLDER) + "pearsonCombinations.csv");
                    ...
                    default:
                        for (DataSeries dataSeries : seriesList) {
                            trainerList.add(new ConfigurationSelectorTrainer(algType, dataSeries, metric, reputation,
                                    tTiming, expList, confList.get(algType)));
                        }
                        break;
                }
            } else {
                switch (algType) {
                    case INV:
                        iManager = new InvariantManager(seriesList, tTiming, expList, metric, reputation,
                                readPossibleIndCombinations());
                        trainerList.addAll(iManager.getInvariants(prefManager.getPreference(DetectionManager.INV_DOMAIN)
                                .equals("ALL")));
                        break;
                    default:
                        for (DataSeries dataSeries : seriesList) {
                            trainerList.add(new ConfigurationFinderTrainer(algType, dataSeries, metric, reputation,
                                    tTiming, expList));
                        }
                        break;
                }
    ...
    private void saveTrainingTimes(LinkedList<? extends Thread> list) {
        ...
        try {
            ...
            writer = new BufferedWriter(new FileWriter(new File(prefManager.getPreference(DetectionManager
                    .OUTPUT_FOLDER) + "/trainingTimings.csv")));
    ...
    private void saveScores(LinkedList<? extends Thread> list) {
        ...
        try {
            writer = new BufferedWriter(new FileWriter(new File(prefManager.getPreference(DetectionManager
                    .SCORES_FILE_FOLDER) + "scores.csv")));
            writer.write("data_series,algorithm_type,reputation_score,metric_score(" + metric.getMetricName()
                    + "),configuration\n");
