...
public class ExperimentVoter extends Thread {
    ...
    private LinkedList<HashMap<AlgorithmVoter, Snapshot>> loadExpAlgSnapshots(ExperimentData expData) {
        ...
        LinkedList<HashMap<AlgorithmVoter, Snapshot>> expAlgMap = new LinkedList<>();
        for (int i = 0; i < expData.getSnapshotNumber(); i++) {
            newMap = new HashMap<>();
            for (AlgorithmVoter aVoter : algList) {
                newMap.put(aVoter, expData.buildSnapshotFor(aVoter.getAlgorithmType(), i, aVoter.getDataSeries(),
                        aVoter.getAlgorithmConfiguration()));
            }
    ...
    private LinkedList<AlgorithmVoter> deepClone(LinkedList<AlgorithmVoter> algorithms) {
        LinkedList<AlgorithmVoter> list = new LinkedList<>();
        try {
    ...
    public void run() {
        ...
        partialVoting = new TreeMap<>();
        voting = new TreeMap<>();
        if (algList.size() > 0) {
            for (HashMap<AlgorithmVoter, Snapshot> anExpSnapMap : expSnapMap) {
                snapVoting = new HashMap<>();
                for (AlgorithmVoter aVoter : algList) {
                    baseTime = AppUtility.readMillis();
                    snapshot = anExpSnapMap.get(aVoter);
     ...
     * @return
     */
    public HashMap<Metric, Double> printVoting(String outFormat, String outFolderName, Metric[] validationMetrics,
                                               double anomalyTreshold, double algConvergence, boolean printOutput) {
        if (printOutput) {
            for (AlgorithmVoter aVoter : algList) {
     ...
     * @param outFolderName     the output folder
     * @param validationMetrics the metrics used for validation and printed in the file
     * @param anomalyThreshold   the anomaly threshold
     * @param algConvergence    the algorithm convergence time (for printing)
     * @param printOutput
     */
    private HashMap<Metric, Double> printExperimentVoting(String outFolderName, Metric[] validationMetrics,
                                                          double anomalyThreshold, double algConvergence,
                                                          boolean printOutput) {
        if (printOutput) {
            printGraphics(outFolderName, anomalyThreshold, algConvergence);
            printText(outFolderName);
        }
        return printMetrics(outFolderName, validationMetrics, anomalyThreshold, printOutput);
    }
     ...
     * @return
     */
    private synchronized HashMap<Metric, Double> printMetrics(String outFolderName, Metric[] validationMetrics,
                                                              double anomalyTreshold, boolean printOutput) {
        PrintWriter pw;
        HashMap<Metric, Double> metResults = new HashMap<>();
        try {
            ...
            if (printOutput) {
                pw = new PrintWriter(new FileOutputStream(new File(outFolderName + "/voter/results.csv"),
                        true));
    ...
    private LinkedList<Snapshot> getSimpleSnapshotList() {
        LinkedList<Snapshot> simpleList = new LinkedList<>();
     ...
     * @param algConvergence  the algorithm convergence time (for printing)
     */
    private void printGraphics(String outFolderName, double anomalyThreshold, double algConvergence) {
        HistogramChartDrawer hist;
        HashMap<String, TreeMap<Double, Double>> voterMap = new HashMap<>();
        ...
        hist = new HistogramChartDrawer("Anomaly Score", "Seconds", "Score", voterMap,
                anomalyThreshold, algConvergence);
    ...
    private void printText(String outFolderName) {
        ...
        try {
            countMap = buildMap();
            writer = new BufferedWriter(new FileWriter(new File(outFolderName + "/voter/" + expName
                    + ".csv")));
            ...
            for (Date timestamp : partialVoting.keySet()) {
                ...
                for (AlgorithmVoter aVoter : algList) {
                    if (partialVoting.get(timestamp).get(aVoter) > 0.0) {
                        countMap.get(aVoter.getLayerType()).replace(aVoter.getAlgorithmType(),
                                countMap.get(aVoter.getLayerType()).get(aVoter.getAlgorithmType()) + 1);
                        partial = partial + aVoter.toString() + "|";
                        count++;
                    }
                }
                writer.write(AppUtility.getSecondsBetween(timestamp, expSnapMap.getFirst().get(algList.getFirst())
                        .getTimestamp()) + ",");
                writer.write(count + ",");
                for (LayerType currentLayer : countMap.keySet()) {
                    for (AlgorithmType algTag : countMap.get(currentLayer).keySet()) {
    ...
    private HashMap<LayerType, HashMap<AlgorithmType, Integer>> buildMap() {
        HashMap<LayerType, HashMap<AlgorithmType, Integer>> map = new HashMap<>();
        for (AlgorithmVoter aVoter : algList) {
            if (!map.keySet().contains(aVoter.getLayerType())) {
                map.put(aVoter.getLayerType(), new HashMap<>());
            }
