...
public class ExperimentPerformance {
     ...
     */
    public ExperimentPerformance(ExperimentData expData, HashMap<String, Integer> algCategories, HashMap<String,
            LinkedList<Integer>> timeSeries) {
    ...
    private String detectorSummary() {
        String dSummary = "";
        for (String detCategory : detectionTimes.keySet()) {
            dSummary = dSummary + detCategories.get(detCategory) + ","
                    + AppUtility.calcAvg(detectionTimes.get(detCategory)) + ","
                    + AppUtility.calcStd(detectionTimes.get(detCategory),
                    AppUtility.calcAvg(detectionTimes.get(detCategory))) + ",";
        }
        return dSummary;
    }
